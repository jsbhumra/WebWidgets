...
const Setup = () => {
  ...
  return (
    <>
      <div className="bgcol w-full min-h-screen overflow-x-hidden">
        <Nav />
        <br></br>
        <div className="relative flex w-[80%] flex-col ms-[10%] items-center text-center z-10 gap-y-4 overflow-x-hidden">
            ...
            {(item) => (
              <Tab key={item.id} title={item.label} className="">
                <Image
                  className="h-[500px]"
                  alt="nextui logo"
                  src={item.pages[currentPage[item.page] - 1].img}
                />
                <Button
                  color="primary"
                  className="text-xl font-medium w-[120px] mt-5"
                  onPress={() => {
                    let newArray = [...currentPage];
                    newArray[item.page] =
                      newArray[item.page] > 1
                        ? newArray[item.page] - 1
                        : newArray[item.page];
